...
public class AuthConfiguration extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        .authorizeRequests()
            ...
            .antMatchers(HttpMethod.GET, "/", "/index", "/addAdmin",
                "/loginAdmin", "/login", "/visualizzaFornitori",
                "/visualizzaCategoria", "/fornitore", "/prodotto","/visualizzaProdotti").permitAll()
            
            .antMatchers(HttpMethod.POST,  "/", "/index", "/addAdmin",
                "/loginAdmin", "/visualizzaFornitori",
                "/visualizzaCategoria", "/fornitore", "/prodotto","/visualizzaProdotti").permitAll()

            // only admin can access the admin page
            .antMatchers(HttpMethod.GET, "/admin", "/inserisciFornitore",
                "/inserisciProdotto").hasAnyAuthority("ADMIN")
            
            .antMatchers(HttpMethod.POST, "/admin", "/inserisciFornitore","/inserisciCategoria",
                "/inserisciProdotto").hasAnyAuthority("ADMIN")
    ...
    public void configureGlobal(AuthenticationManagerBuilder auth) throws Exception {
        auth.jdbcAuthentication().dataSource(this.buildDatasource())
                ...
                .usersByUsernameQuery("SELECT username, password, 1 as enabled FROM admins WHERE username=?");
    }
